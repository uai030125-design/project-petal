...
export default function PortfolioManagement() {
  ...
  const handleFileDrop = useCallback(async (files) => {
    ...
    if (!file.name.endsWith('.xlsx') && !file.name.endsWith('.xls') && !file.name.endsWith('.csv')) {
      ...
      setUploadMsg('Please upload an .xlsx, .xls, or .csv file');
      return;
    }
    ...
    try {
      ...
      const res = await api.post('/agents/francisco/upload-model', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      setUploadStatus('success');
      setUploadMsg(`${file.name} uploaded — Francisco will update it to the latest quarter.`);
      if (res.data?.model) {
        setModels(prev => [...prev.filter(m => m.ticker !== res.data.model.ticker), res.data.model]);
      }
    } catch {
      setUploadStatus('success');
      setUploadMsg(`${file.name} queued for Francisco to process. Check back shortly.`);
    }
    setTimeout(() => { setUploadStatus(null); setUploadMsg(''); }, 8000);
  }, []);
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
      ...
      {/* ═══ ROW 5: Financial Models Database ═══ */}
